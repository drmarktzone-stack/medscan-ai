...
/** בונה אינדקס שלבים ומאתר את השלב הנוכחי. דטרמיניסטי. */
export function resolveStep(protocol, currentStepId) {
  const steps = protocol?.steps ?? [];
  const index = new Map(steps.map((s) => [s.step_id, s]));

  if (!steps.length) {
    return { step: null, index, error_he: 'הפרוטוקול אינו מכיל שלבים.' };
  }
  ...
  // הסתעפויות שמפנות לשלב שאינו קיים — פגם בפרוטוקול עצמו, לא במודל.
  // עדיף לגלות אותו כאן מאשר ברגע שרופא/ה ילחץ/תלחץ עליו.
...
/**
 * ולידציה ייחודית לפרוטוקול: כל מזהה בפלט חייב להתקיים בפועל.
 * זו הבדיקה שמונעת "צעד מומצא", שהוא הכשל החמור ביותר במנוע הזה.
 */
export function validateProtocolOutput({ output, protocol, stepIndex, allowedActions }) {
  ...
  if (outStepId && !stepIndex.has(outStepId)) {
    violations.push({
      ...
      message_he:
        ...
        'שלב מומצא נראה לרופא/ה כחלק מהפרוטוקול — זהו הכשל החמור ביותר במנוע זה.',
    });
  }
  ...
  for (const b of output?.branch_options ?? []) {
    if (!stepIndex.has(b.next_step_id)) {
      violations.push({
        ...
        severity: 'block',
        message_he:
          `ההסתעפות מפנה לשלב "${b.next_step_id}" שאינו קיים בפרוטוקול.`,
      });
    }
  }
  ...
  for (const a of output?.current_step?.actions_he ?? []) {
    ...
    // התאמה חלקית: המודל עשוי לנסח מחדש. דורשים חפיפה מהותית.
    const overlaps = [...allowed].some(
      (orig) => orig.includes(clean) || clean.includes(orig)
    );
    if (!overlaps) {
      violations.push({
