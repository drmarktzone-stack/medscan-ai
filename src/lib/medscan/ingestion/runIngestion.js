...
export async function runIngestion({
  ...
  // דילוג על נושאים שכבר יובאו — הרצה חוזרת אינה משכפלת
  const existing = new Set((await listKbEntity('KnowledgeTopic')).map((t) => t.topic_key));
  ...
  for (const unit of units) {
    ...
    if (existing.has(unit.topic_key) && unit.part === 1) {
      summary.skipped += 1;
    ...
    try {
      ...
      if (error || !extraction) {
        ...
        if (!dryRun) {
          const { saved, failed } = await saveExtraction(kept);
